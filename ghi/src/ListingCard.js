        ...
        <div className="container flex justify-center mx-4 max max-w-4xl">
            <div className="w-64 sm:w-80 overflow-hidden m-5 max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-slate-50 dark:border-gray-700">
                <div className="rounded-t-lg overflow-hidden h-48 object-fill flex items-center justify-center">
                    <img className="rounded-t-lg object-fill w-full h-full" src={picture} alt="product image" />
                </div>
                <a href="#">
                    <h5 className="mx-2 text-lg font-bold tracking-tight text-gray-900 dark:text-gray-950 truncate">{name}</h5>
                </a>
                <span className="mx-2 bg-blue-100 text-blue-800 text-xs font-semibold mr-2 p-1 rounded dark:bg-green-200 dark:text-gray-950">{isNew ? "New" : "Used"}</span>
                <div className="flex items-center justify-between p-2">
                    <span className="text-xl font-semibold text-gray-900 dark:text-gray-950">{formatter.format(price)}</span>
                    <a href="#" className="text-white bg-green-800 hover:bg-green-900 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
